...
// A label that resolves through PagesWithLabel must render the matched page. The one-page document defines no
// /PageLabels tree, so its only page falls back to the decimal ordinal label "1".
func TestExtractWithLabel(t *testing.T) {
	dir := t.TempDir()
	t.Chdir(dir)
	path := filepath.Join(dir, "one.pdf")
	if err := os.WriteFile(path, []byte(onePagePDF), 0o600); err != nil {
...
// Requesting a label that no page has must fail with the no-match error rather than reaching RenderPage, and must not
// leave a PNG behind.
func TestExtractWithUnknownLabel(t *testing.T) {
	dir := t.TempDir()
	t.Chdir(dir)
	path := filepath.Join(dir, "one.pdf")
	if err := os.WriteFile(path, []byte(onePagePDF), 0o600); err != nil {
	...
	if err := extract(path, "2", ""); err == nil {
		t.Fatal("expected an error for a label that no page has")
	}
	if _, err := os.Stat(filepath.Join(dir, "page0.png")); !os.IsNotExist(err) {
		t.Errorf("expected no page0.png to be written for a label that no page has, got %v", err)
	}
}
