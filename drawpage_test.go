...
func TestDrawPage(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(fmt.Sprintf("%s-p%d-%ddpi", tc.name, tc.page, tc.dpi), func(t *testing.T) {
			...
			// ~3x headroom over the measured worst (0.102% over Δ24, 2.155% over Δ8) so a genuine regression still
			// trips.
			if pctOver24 := 100 * float64(over24) / float64(total); pctOver24 > 0.31 {
				t.Fatalf("DrawPage output diverges from RenderPage: %.4f%% of pixels over Δ24", pctOver24)
			}
			if pctOver8 := 100 * float64(over8) / float64(total); pctOver8 > 6.5 {
