...
// resolveEncoding builds a simple font's code→glyph-name table (ISO 32000-2 9.6.5). The base is the font's built-in
// encoding: the embedded program's own table when it declares one (builtin non-nil, Type 1), else the generated tables
// for the standard Symbol and ZapfDingbats fonts, else StandardEncoding. An /Encoding name or dictionary (whose
// /BaseEncoding, then /Differences, apply in order) overrides it. The returned table is never mutated after Load;
// unmodified base tables are shared.
