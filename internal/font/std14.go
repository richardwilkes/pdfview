...
// std14Aliases maps the well-known non-canonical BaseFont names to the 14 standard fonts' canonical
// PostScript names. The set follows the conventional mapping deployed viewers share (compare pdf.js's
// getStdFontMap, Apache-2.0); names not listed fall back to the flag/name heuristics in standard14Name.
...
// standard14Name maps a (subset-prefix-stripped) BaseFont name plus descriptor flags to the canonical
// standard-14 font that substitutes for it. The mapping is deterministic and never consults system fonts:
// exact aliases first, then style parsing of the name (",Bold", "-Oblique", ...) combined with the
// fixed-pitch/serif/italic descriptor flags.
...
// nimbusMetrics holds the oracle-pinned quad metrics of MuPDF's substitute fonts in 1000-unit em space,
// keyed by canonical standard-14 name. Every value was recovered exactly (integer font units) from the
// std14-styles golden's 50-pt probe quads, except ZapfDingbats: MuPDF extracts no searchable Unicode for it,
// so its entry carries the Adobe ZapfDingbats AFM FontBBox (yMin -143, yMax 820) as a documented,
// deterministic stand-in until something pins it.
