...
// pageGeom is a page's effective display geometry: the normalized CropBox ∩ MediaBox in PDF space (y-up, arbitrary
...
// inheritedAttrs carries the inheritable page attributes (ISO 32000-2 7.7.3.4) down the page-tree walk. Values are kept
// unresolved until a leaf is reached; a node's own entry overrides whatever an ancestor supplied.
...
// resolveGeom converts the inherited attributes of one page into its effective geometry: /MediaBox (falling back to US
// Letter when absent or degenerate), intersected with /CropBox when one is present (an empty intersection falls back to
// the MediaBox), plus the normalized rotation. Verified against MuPDF across box origins and all four rotations.
...
// normalizeRotate maps an arbitrary /Rotate value to 0, 90, 180, or 270. The value is first normalized into [0, 360),
// then rounded to the nearest multiple of 90 (ties round up), matching MuPDF's observed handling of out-of-spec values
// (probed: 45, 100, 450, -90, and -450 all display like 90/270).
...
// displaySize returns the page's displayed width and height in PDF points: the effective box extent, with the axes
// swapped when the rotation is 90 or 270.
