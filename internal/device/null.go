...
// Null is a Device that ignores every operation, so the interpreter can be driven without a consumer — most importantly
// by FuzzContent, which exercises the interpreter against hostile content streams without paying for rasterization.
type Null struct{}
