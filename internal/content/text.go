...
//
// with the glyph advance applied to Tm after each glyph: tx = ((w0 − TJ/1000)·Tfs + Tc + Tw)·Th, where Tw applies only
// to single-byte code 32. Text-state parameters live in the graphics state (textParams); Tm/Tlm live on the interpreter
// and reset at BT. Ops arriving outside BT..ET are processed against the stream's initial identity matrices, the
// leniency deployed viewers apply to sloppy content.
...
// opEndText implements ET, and force-closes truncated text objects at stream end. Any accumulated text-clip runs are
// finalized into a single device clip level, popped by Q/unwind like every other clip.
...
// opTJ implements TJ: strings show, numbers kick the text matrix by −n/1000 in text space. One run spans the whole
// array.
func (in *interp) opTJ() {
...
// is then skipped entirely, matching the oracle's no-font recovery).
...
// emitRun dispatches a completed run per the text render mode (ISO 32000-2 9.3.6): modes 0-2 paint, 3 is recorded but
// invisible, and 4-7 additionally (or only) accumulate the text clip. Pattern color spaces paint only once an scn has
// selected a pattern, exactly like path painting. Type 3 runs paint by interpreter recursion into their CharProcs
// instead of outline drawing.
...
func (in *interp) emitType3Run(run *device.TextRun, paint bool, mode int) {
	...
	// The whole run composites through the soft mask once (the mask gates the run as one object, like MuPDF's
	// per-text-object mask application); the procs execute with the mask lifted so their individual paints do not each
	// re-apply it.
	in.masked(in.gs.fillAlpha, func() {
		in.dev.FillText(run, in.fillPaint())
...
// execType3Glyph executes one glyph's charproc with CTM = FontMatrix · Trm, its own resource frame, and fresh
// per-stream state — the same discipline as form XObjects, sharing their depth cap and budget.
