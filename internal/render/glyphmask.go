...
// the profile, so ordinary fill-mode text rasterizes each distinct glyph appearance once into an Alpha8 coverage plane
// and blits it at integer device positions, like MuPDF's glyph bitmap cache. An entry is keyed by the glyph identity,
// the full float32 Trm linear part, and the exact subpixel phase of the glyph origin, so a blit reproduces the coverage
// the direct fill would produce bit-for-bit; only the final color application can differ by ±1 rounding (see
// TestGlyphBlitMatchesDirectFill). The first render of a page mostly misses (each glyph instance has its own x phase)
// and re-renders hit 100%, the warm protocol real consumers (re-render on scroll/zoom) care about. Entries live in the
// document's budgeted store when one is wired, else in a per-render map. Neither cache's occupancy may steer a glyph
// onto a different path — see glyphMask.
...
// sizes text comes out lighter than the platform rasterizers users compare against — CoreGraphics dilates glyph
// outlines ("font smoothing") and FreeType darkens CFF stems for the same reason. When enabled, glyph fills are drawn
// stroke-and-fill with a sub-pixel pen, so sub-pixel features saturate to full ink while the geometry stays within half
// the pen width of exact. The width was fitted against macOS Preview renders of embedded-CFF body text at 72/150/300
// dpi (ink coverage matches within 1% relative at each): linear in the glyph's device pixel size at small sizes, capped
// at one device pixel so display sizes gain only a hair of weight. The stroke is cached with the coverage plane, so
// warm renders pay nothing.
const (
...
// stemDarkenWidth returns the stroke-and-fill pen width, in device pixels, for a glyph whose glyph→device linear part
// is (a, b, c, d) — zero when darkening is off or the transform is degenerate. sqrt|det| is the geometric mean of the
// glyph's device pixel size (its ppem for an unrotated square transform), stable under rotation and reasonable under
// skew. Computed in float64 so finite float32 inputs cannot overflow the products.
...
// maxGlyphMaskBytes caps the live coverage planes the per-device cache holds. An entry count bounds nothing: one plane
// may be maxGlyphMaskDim² = 64 KiB, so a few thousand entries is hundreds of megabytes. 16 MiB holds tens of thousands
// of text-size glyphs, well past what any real page draws.
const maxGlyphMaskBytes = 16 << 20

// glyphMaskKey identifies one cached glyph coverage plane. Distinct store key type per the store's kind-separation
// rule. dark is part of the key even though the darkening width derives from a–d: the per-device map and the store
// both outlive a render, darkening can be toggled between renders, and a plane rasterized under one setting must never
...
// DrawImage route, created lazily since the direct composite (the common route) reads plane straight.
...
// path. Glyphs the mask path cannot handle (oversized, degenerate) accumulate into a leftover outline filled like the
// slow path.
func (d *Device) blitTextRun(run *device.TextRun, p device.Paint) bool {
	...
	// rectangle, the draw target is the base surface at identity, so a glyph whose mask rect stays inside the tracked
	// clip interior composites straight into the surface pixmap. canvas's Alpha8 image draws run the general float
	// shader pipeline (no sprite fast path exists for Alpha8), which dominated warm renders. Everything else goes through
	// DrawImage so canvas applies clip and layer.
	...
	for i := range run.Glyphs {
		...
		// Trm passed IsFinite, but ox/oy can still be ~3.4e38, and int(ox) would then overflow (architecture-defined).
		// Such a glyph folds into the leftover outline like any other the fast path declines. Mirrors renderGlyphMask's
		// 1<<24 clamp.
		...
		if mask == nil || mask.plane == nil || originOverflow {
			// These glyphs are the ones renderGlyphMask declined, possibly BECAUSE the outline's transformed corners were
			// non-finite or past 1<<24, so the bounds test the merged-outline path applies (textOutline) applies here too;
			// otherwise a glyph the slow path skips would cross into canvas with ±Inf/NaN coordinates.
	...
	if leftover != nil && !leftover.IsEmpty() {
		if cpaint, ok := d.preparePaint(p, nil); ok {
			// The run-level pen FillText applies, so a declined glyph darkens exactly as on the slow path.
...
//
// Which glyphs take the mask path can never depend on cache occupancy: the store is a pure cache, and a budget of any
// size must leave rendered output byte-identical (see the store package comment, pinned by TestCacheBudget). A blit and
// the merged-outline fill agree only within ±1 of compositing rounding, so a cache-driven fallback would make a page's
// pixels a function of its budget. The cost of a miss is therefore kept low rather than avoided: everything a miss
// touches beyond the plane itself — the scratch surface, its clear, the transformed outline, the fill paint, the image
// wrapper — is reused, deferred, or done in place.
func (d *Device) glyphMask(f *font.Font, g *device.Glyph, gp *path.Path, fx, fy float32) *glyphMask {
	...
	if d.glyphMaskBytes+size > maxGlyphMaskBytes {
		// The map has no eviction; refusing further entries would retire the cache for the rest of the render, so every
		// later glyph would miss and rebuild with no prospect of a hit. Dropping the map keeps the cap on live planes and
		// lets the page go on caching. Retention is invisible to output: a hit reproduces the plane a miss would render.
		clear(d.glyphMasks)
		d.glyphMaskBytes = 0
	}
...
//
// The document store holds them only when it carries a byte budget to evict under. Plane keys accumulate with GLYPHS
// DRAWN rather than with distinct resources, and under the unlimited budget New(buffer, 0) selects nothing is ever
// evicted, so "no limit" would mean memory proportional to every glyph the document ever rendered. The per-device map
// bounds itself (maxGlyphMaskBytes) and, with a store wired, survives Reset, so a re-render at the same size still
// hits; a re-render at a different size could not hit either way, since the Trm in the key changes with the scale.
...
func (d *Device) renderGlyphMask(g *device.Glyph, gp *path.Path, fx, fy float32) (mask *glyphMask, size uint64) {
	...
	// Reject finite device bounds that dwarf any real glyph: the floor/ceil below would otherwise overflow int
	// (architecture-defined) and the w/h subtraction could wrap back into a small positive value that slips past the
	// maxGlyphMaskDim gate as an all-zero plane, dropping the glyph instead of taking the outline fallback. Mirrors
	// rectInterior's 1<<24 clamp.
	const maxReasonable = 1 << 24
	...
	// Zero just the w×h corner this mask uses (the surface is sized for the largest glyph seen) straight in the backing
	// store: a canvas Clear allocates a paint and a blitter per miss, and nothing ever snapshots this surface, so there is
	// no copy-on-write state the canvas would need to see invalidated.
	for row := range h {
	...
	local.F -= float32(my0)
	m := matrix(local)
	// One scratch path and one paint serve every miss; with the outline storage retained across rewinds the transformed
	// copy costs only its points. The fill below calls into canvas, never back into the device, and gp is a cached
	// glyph-space outline, never this path, so the scratch cannot be shared out from under itself.
	if d.maskPath == nil {
		d.maskPath = path.New()
	...
	// The canvas image stays unbuilt: nearly every glyph takes the direct pixmap composite, which reads plane straight, so
	// image() wraps it the first time a glyph goes through DrawImage. The size estimate still charges for the wrapper,
	// since a mask that reaches DrawImage keeps one for the entry's life.
...
// Alpha8 plane. It returns nil rather than panicking when pm is nil, the same guard compositeMask and Pixels apply.
...
// pixmap at integer device position (x0, y0). The caller guarantees the blit lies inside every open clip and that no
// layer or matrix is in effect, so this is exactly the draw canvas would perform, minus the general image pipeline's
// per-pixel float cost. out = src·c/255 + dst·(255−c)/255 per channel, single-rounded.
...
// matching run of coverage bytes. It is the default behind compositeMaskSpanFn and the vector kernel's fallback for
// short spans.
...
// maskScratchSurface returns a scratch surface at least w×h, growing the cached one as needed; the caller clears the
// region it uses.
