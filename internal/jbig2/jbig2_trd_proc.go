...
//   - Charges the text region's own bitmap against the decode's cumulative pixel budget (embedded.go) before
//     allocating it, and hands that budget to the refinement proc it drives, whose bitmap is charged there.
//   - Restored PDFium's hard failure on a nil symbol in the arithmetic path (`if (!pIBI) return nullptr`). This
//     translation skipped the instance instead, and because the arithmetic decoder never runs out of input — it
//     feeds 0xFF forever past the end — the instance counter then never advanced and the loop spun forever on a
//     dictionary holding a nil symbol. The Huffman path keeps upstream's skip, which is what PDFium does there
//     (`if (!IBI) continue`), and terminates because its bit reader does report exhaustion.
//
...
func (t *TRDProc) DecodeHuffman(stream *BitStream, grContexts []ArithCtx) (*Image, error) {
	if err := t.budget.charge(int64(t.SBW), int64(t.SBH)); err != nil {
		return nil, err
	}
...
func (t *TRDProc) DecodeArith(arithDecoder *ArithDecoder, grContexts []ArithCtx, ids *IntDecoderState) (*Image, error) {
	...
	for NINSTANCES < t.SBNUMINSTANCES {
		...
		for {
			...
			if IBI == nil {
				return nil, errors.New("text region symbol is nil")
			}
			WI := uint32(IBI.width)
			HI := uint32(IBI.height)
			if !t.TRANSPOSED && (t.REFCORNER == JBig2CornerTopRight || t.REFCORNER == JBig2CornerBottomRight) {
