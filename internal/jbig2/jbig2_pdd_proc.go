...
//   - Charges the pattern cells this file splits out of the collective bitmap against the decode's cumulative pixel
//     budget (embedded.go) before allocating them, and hands that budget to the generic-region proc that decodes the
//     collective bitmap, which charges it there.
//
...
func (p *PDDProc) DecodeArith(arithDecoder *ArithDecoder, gbContexts []ArithCtx) (*PatternDict, error) {
	...
	if err := p.budget.charge(int64(p.GRAYMAX)+1, int64(p.HDPW)*int64(p.HDPH)); err != nil {
		return nil, err
	}
