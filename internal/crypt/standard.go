...
func (h *Handler) deriveRC4(pw []byte) (fileKey []byte, user, owner bool) {
	...
	// The owner password decrypts /O to recover the padded user password, which is then validated as a user
...
// userKeyValid reports whether key reproduces the stored /U entry (Algorithm 6): all 32 bytes for R2, the
// first 16 for R3+ (the remaining bytes of /U are arbitrary padding).
...
// userPasswordFromOwner recovers the padded user password from /O using the owner password (Algorithm 7,
// reversing the RC4 rounds Algorithm 3 applied).
...
// deriveAES256 authenticates password against an R5/R6 document (Algorithm 2.A), returning the file encryption
// key on success.
...
// hash256 is the R5/R6 password hash: a plain SHA-256 for R5, and the hardened iterated hash of Algorithm 2.B
// for R6. salt is the 8-byte validation or key salt; udata is empty for the user password and the 48-byte /U
// for the owner password.
...
// hash2B is Algorithm 2.B (ISO 32000-2 7.6.4.3.4), the R6 hardened hash. k is the initial SHA-256 of
// (pw + salt + udata); the round loop then repeatedly AES-encrypts and re-hashes until the termination
// condition is met, returning the first 32 bytes.
...
// saslPrep applies the parts of the SASLprep profile (RFC 4013) that matter for the PDF passwords seen in
// practice: the UTF-8 bytes are truncated to 127 bytes (ISO 32000-2 7.6.4.3.3). Full stringprep normalization
// is not performed; ASCII passwords — effectively all of them — are unaffected.
