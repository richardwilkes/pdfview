...
// terminal image codec — none (raw samples), DCTDecode (stdlib image/jpeg, including CMYK/YCCK with the Adobe
// APP14 transform), or CCITTFaxDecode (x/image/ccitt) — produces component samples. Samples are unpacked per
...
// Errors reported by this package. The interpreter treats every decode failure the same way — the image is
// skipped and the page keeps rendering — so these exist for tests and logs rather than control flow.
var (
...
// maxImagePixels is the hard cap on decoded image pixels (width × height), chosen so a 600-dpi letter-size
// bilevel scan (~34 Mpx) still decodes while the RGBA expansion stays bounded (~256 MB worst case). Larger
// images are skipped (rendered blank), never partially decoded.
const maxImagePixels = 1 << 26

// maxPixelsFor returns the pixel budget for an image whose encoded payload is dataLen bytes:
// min(maxImagePixels, max(2^22, 8192×dataLen)). The proportional term stops hostile dictionaries from claiming
// huge dimensions over a handful of payload bytes (missing samples read as zeros, so the only effect of such a
// claim is allocation), while the floor and the generous per-byte multiplier accommodate legitimately extreme
// compression: an all-white fax page compresses to a few bytes per row under CCITT G4.
...
type Image struct {
	// Pix holds the decoded pixels, row-major from the image's top-left sample: straight (non-premultiplied)
	// RGBA, 4 bytes per pixel, when Stencil is false; one coverage byte per pixel (255 = paint) when Stencil is
	// true.
	Pix    []byte
	Width  int
	Height int
...
// DecodeXObject decodes an image XObject. resources is the resource dictionary in scope at the Do operator,
// consulted (leniently — the spec reserves this for inline images) when /ColorSpace is a name that is not a
// device space.
...
// decoder carries one image's decode state: the dictionary, the payload with non-image filters already applied,
// and the terminal image codec (empty for raw samples).
type decoder struct {
	d     *cos.Document
	dict  cos.Dict
	parms cos.Dict
...
// entry resolves dict[full], falling back to the abbreviated inline-image key. Both spellings are accepted for
// both image kinds (PDF 2.0 allows full names inline; abbreviations elsewhere are harmless leniency).
...
func (dec *decoder) run() (*Image, error) {
	...
	if dec.boolEntry("ImageMask", "IM") {
		// A stencil mask: one bit per sample regardless of any declared BitsPerComponent or ColorSpace
		// (ISO 32000-2 8.9.6.2); Decode [1 0] flips polarity.
...
// colorSpace resolves the image's color space: a direct name or array first, then — for names — the
// /ColorSpace subdictionary of the resource dictionary in scope (the inline-image rule, extended to XObjects as
// leniency). A /Pattern space is invalid for images.
