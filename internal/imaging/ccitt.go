...
// parameters' column count, h rows tall. The output bit convention matches PDF's decoded-data contract
// directly: with BlackIs1 false (the default) a 0 bit is black, with BlackIs1 true a 1 bit is black — the
// /Decode array and color space then interpret the bits like any other 1-bpc samples. K selects the coding
// scheme: negative is pure two-dimensional (Group 4), zero is one-dimensional (Group 3); positive (mixed
// one/two-dimensional Group 3) is attempted as Group 3, whose two-dimensional lines then terminate the decode
// early — the truncation leniency below completes the image with white. Truncated or damaged payloads keep the
// rows that decoded; the remainder is filled with white, the way deployed viewers degrade.
func (dec *decoder) decodeCCITT(h int) (data []byte, cols int, err error) {
	...
	// The x/image reader emits 1 for white and 0 for black; its Invert option flips that, which is exactly
	// BlackIs1's contract for the decoded data.
